<template>
    <CreateUnits v-if="SettingRepository.createDailog" />
    <UpdateUnits v-if="SettingRepository.updateDailog" />
    <toolbar title="Setting-" subtitle="Unit" />
                        ...
                        <v-row>
                            <v-col>
                                <v-data-table-server
                                    ...
                                    :items="SettingRepository.units"
                                    :loading="SettingRepository.loading"
                                    :search="SettingRepository.ServiceSearch"
                                    item-value="id"
                                    @update:options="
                                        SettingRepository.FetchUnitsData
                                    "
                                    :item-key="SettingRepository.units"
                                    itemKey="id"
                                    hover
                                >
                                    <template
...
const headers = [
    { title: "UNIT", key: "name", sortable: false },
    { title: "SHORT NAME", key: "shortName", sortable: false },
...
const deleteItem = (id) => {
    SettingRepository.DeleteUnit(id);
};
const editItem = (id) => {
    SettingRepository.unit = {};
    if (Object.keys(SettingRepository.unit).length === 0) {
        SettingRepository.FetchUnitData(id)
            .then(() => {
